...
function SearchInterface() {
    ...
    useEffect(() => {
        axios
            .get('http://localhost:8000/api/search-message/')
            .then((response) => {
                setMessage(response.data.message);
            })
            .catch((error) => {
                console.error(
                    'There was an error fetching the search interface message:',
                    error
                );
            });
    }, []);
        ...
        <div>
            ...
            <div>
                <input
                    ...
                    onKeyDown={(e) => {
                        // do a search on enter key press
                        if (e.key === 'Enter') {
                            handleSearch();
                        }
                    }}
